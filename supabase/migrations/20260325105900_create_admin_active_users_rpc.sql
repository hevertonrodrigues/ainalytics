-- ============================================================
-- RPC: get_admin_active_users
-- Returns all active users with aggregated stats computed
-- in SQL to avoid PostgREST 1000-row limits.
-- ============================================================
...
BEGIN
  ...
  FROM (
    ...
    -- Geo analyses aggregate (across all companies of tenant)
    LEFT JOIN LATERAL (
      SELECT
    ...
    -- Answers aggregate (no row limit!)
    LEFT JOIN LATERAL (
...
-- Only service_role (edge functions with admin client) should call this
REVOKE ALL ON FUNCTION public.get_admin_active_users() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_admin_active_users() TO service_role;
