...
UPDATE models SET
  ...
  price_per_output_token = 0.000010,
...
UPDATE models SET
  ...
  price_per_output_token = 0.000020,
  pricing_updated_at     = now()
WHERE slug = 'gpt-5.4'
  AND platform_id = (SELECT id FROM platforms WHERE slug = 'openai');
...
UPDATE models SET
  ...
  price_per_output_token = 0.0000006,
  pricing_updated_at     = now()
WHERE slug LIKE 'gemini-2.5-flash%'
  AND platform_id = (SELECT id FROM platforms WHERE slug = 'gemini');
...
-- Perplexity (set reasonable defaults)
UPDATE models SET
  price_per_input_token  = 0.000001,
  price_per_output_token = 0.000005,
  pricing_updated_at     = now()
WHERE platform_id = (SELECT id FROM platforms WHERE slug = 'perplexity');
