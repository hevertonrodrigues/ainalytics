...
`{lang}` ∈ `pt | es | en` (path segment, no fallback — clients must send it explicitly).
...
```json
{
  ...
  "seo": {
    ...
    "twitterHandle":    "@indexai",                  // nullable
    "publisher": {
      "name": "Ainalytics",
    ...
    "publishedTime":    "2026-04-23T12:00:00Z",      // article only
    "modifiedTime":     "2026-04-24T09:00:00Z",      // article only
    "robots":           "index,follow",
    "structuredData":   { /* JSON-LD; pre-built for /trending and news detail */ }
  }
}
```
    ...
    "items": [
      {
        ...
        "modifiedAt": "2026-04-24T09:00:00Z",
        "readTimeMinutes": 12,
...
fewer than `limit` articles are flagged as trending.
...
```json
{
  "data": {
    ...
    "featured": { /* news teaser, the hero card */ },
...
| `422`  | `{ "error": "validation_failed", "message": "Consent is required" }`|
| `400`  | `{ "error": "bad_request", "message": "Invalid email" }`            |
...
Codes used: `bad_request`, `invalid_filter`, `unsupported_lang`, `not_found`,
`gone`, `validation_failed`, `rate_limited`, `internal_error`,
`upstream_unavailable`. Errors set `Cache-Control: no-store`.
...
| Sector slug             | Subsector slugs |
| ----------------------- | --------------- |
| `financial-services`    | `banks`, `fintech-neobanks`, `insurance`, `payments-wallets`, `wealth-investing` |
| `healthcare`            | `hospitals`, `health-plans`, `telehealth`, `pharma-brands`, `labs-diagnostics` |
| `education`             | `universities`, `business-schools`, `online-learning`, `k12-schools`, `bootcamps` |
| `retail-ecommerce`      | `marketplaces`, `grocery-ecommerce`, `electronics-retail`, `home-goods`, `pharmacy-retail` |
| `travel-hospitality`    | `hotels-resorts`, `airlines`, `otas-metasearch`, `vacation-rentals`, `tours-experiences` |
| `consumer-brands`       | `beauty-cosmetics`, `luxury-fashion`, `apparel-fast-fashion`, `sportswear`, `jewelry-watches` |
| `technology`            | `ai-platforms`, `cloud`, `cybersecurity`, `saas`, `mobile-operators` |
| `real-estate`           | `residential-developers`, `property-portals`, `brokerages`, `coworking`, `rental-platforms` |
| `automotive-mobility`   | `automakers`, `ev-brands`, `ride-hailing`, `car-rental-leasing`, `charging-networks` |
| `food-restaurants`      | `grocery-chains`, `quick-commerce`, `fast-food`, `coffee-chains`, `casual-dining` |
